...
namespace ChessBrowser
{
    partial class Form1
    {
        ...
        private void UploadGamesToDatabase(string PGNfilename)
        {
            ...
            List<ChessGame> games = reader.ReadPGN(PGNfilename);
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    MySqlCommand insertPlayer = new MySqlCommand();
                    MySqlCommand insertGame = new MySqlCommand();
                    MySqlCommand insertEvent = new MySqlCommand();
                    insertPlayer.Connection = conn;
                    insertGame.Connection = conn;
                    insertEvent.Connection = conn;

                    insertPlayer.CommandText = "INSERT INTO Players(Name, Elo) VALUES(@name, @elo) ON DUPLICATE KEY UPDATE pID = LAST_INSERT_ID(pID), Elo = GREATEST(VALUES(Elo), Elo)";
                    insertGame.CommandText = "INSERT INTO Games(Round, Result, Moves, BlackPlayer, WhitePlayer, eID) VALUES(@round, @result, @moves, @blackplayer, @whiteplayer, @eid)";
                    insertEvent.CommandText = "INSERT INTO Events(Name, Site, Date) VALUES(@name, @site, @date) ON DUPLICATE KEY UPDATE eID = LAST_INSERT_ID(eID), Name = @name, Site = @site, Date = @date";
                    insertPlayer.Prepare();
                    insertGame.Prepare();
                    insertEvent.Prepare();

                    insertPlayer.Parameters.Add("@name", MySqlDbType.VarChar);
                    insertPlayer.Parameters.Add("@elo", MySqlDbType.UInt16);

                    insertGame.Parameters.Add("@round", MySqlDbType.VarChar);
                    insertGame.Parameters.Add("@result", MySqlDbType.String);
                    insertGame.Parameters.Add("@moves", MySqlDbType.VarChar);
                    insertGame.Parameters.Add("@blackplayer", MySqlDbType.UInt16);
                    insertGame.Parameters.Add("@whiteplayer", MySqlDbType.UInt16);
                    insertGame.Parameters.Add("@eid", MySqlDbType.UInt16);

                    insertEvent.Parameters.Add("@name", MySqlDbType.VarChar);
                    insertEvent.Parameters.Add("@site", MySqlDbType.VarChar);
                    insertEvent.Parameters.Add("@date", MySqlDbType.DateTime);

                    foreach (ChessGame game in games)
                    {
                        insertPlayer.Parameters["@name"].Value = game.BlackPlayer;
                        insertPlayer.Parameters["@elo"].Value = game.BlackElo;
                        insertPlayer.ExecuteNonQuery();
                        long blackID = insertPlayer.LastInsertedId;
                        insertPlayer.Parameters["@name"].Value = game.WhitePlayer;
                        insertPlayer.Parameters["@elo"].Value = game.WhiteElo;
                        insertPlayer.ExecuteNonQuery();
                        long whiteID = insertPlayer.LastInsertedId;

                        insertEvent.Parameters["@name"].Value = game.EventName;
                        insertEvent.Parameters["@site"].Value = game.Site;
                        insertEvent.Parameters["@date"].Value = game.EventDate;
                        insertEvent.ExecuteNonQuery();
                        long eventID = insertEvent.LastInsertedId;

                        insertGame.Parameters["@round"].Value = game.Round;
                        insertGame.Parameters["@result"].Value = game.Result;
                        insertGame.Parameters["@moves"].Value = game.Moves;
                        insertGame.Parameters["@blackplayer"].Value = blackID;
                        insertGame.Parameters["@whiteplayer"].Value = whiteID;
                        insertGame.Parameters["@eid"].Value = eventID;
          ...
          string winner, bool useDate, DateTime start, DateTime end, bool showMoves)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    int filterCount = 0;
                    if (showMoves)
                        query.CommandText = "SELECT Events.Name as EventName, Site, Date, w.Name as WhitePlayer, w.Elo as WhiteElo, b.Name as BlackPlayer, b.Elo as BlackElo, Result, Moves FROM Games NATURAL JOIN Events JOIN Players w ON w.pID = WhitePlayer JOIN Players b ON b.pID = BlackPlayer WHERE ";
                    else
                        query.CommandText = "SELECT Events.Name as EventName, Site, Date, w.Name as WhitePlayer, w.Elo as WhiteElo, b.Name as BlackPlayer, b.Elo as BlackElo, Result FROM Games NATURAL JOIN Events JOIN Players w ON w.pID = WhitePlayer JOIN Players b ON b.pID = BlackPlayer WHERE ";

                    if (white != "")
                    {
                        AddAndToQuery(ref query, filterCount);
                        filterCount++;
                        query.CommandText += "w.Name = '" + white + "'";
                    }
                    if (black != "")
                    {
                        AddAndToQuery(ref query, filterCount);
                        filterCount++;
                        query.CommandText += "b.Name = '" + black + "'";
                    }
                    if (opening != "")
                    {
                        AddAndToQuery(ref query, filterCount);
                        filterCount++;
                        query.CommandText += "Moves LIKE '" + opening + "%'";
                    }
                    if (winner != "")
                    {
                        AddAndToQuery(ref query, filterCount);
                        filterCount++;
                        query.CommandText += "Result = '";
                        switch (winner)
                        {
                            case "White":
                                query.CommandText += "W'";
                    ...
                    if (useDate)
                    {
                        AddAndToQuery(ref query, filterCount);
                        filterCount++;
                        query.CommandText += "Date <= '" + String.Format("{0:yyyy/MM/dd}", end) + "' AND Date >= '" + String.Format("{0:yyyy/MM/dd}", start) + "'";
                    ...
                    while (reader.Read())
                    {
                        ...
                        string whitePlayer = reader.GetString("WhitePlayer");
                        string blackPlayer = reader.GetString("BlackPlayer");
                        int wElo = reader.GetUInt16("WhiteElo");
                        int bElo = reader.GetUInt16("BlackElo");
                        char result = reader.GetChar("Result");

                        parsedResult += "Event: " + eventName + newLine;
                        parsedResult += "Site: " + site + newLine;
                        parsedResult += "Date: " + date.ToString() + newLine;
                        parsedResult += "White: " + whitePlayer + " (" + wElo.ToString() + ")" + newLine;
                        parsedResult += "Black: " + blackPlayer + " (" + bElo.ToString() + ")" + newLine;
                        parsedResult += "Result: " + result + newLine;

                        if (showMoves)
                        {
                            string moves = reader.GetString("Moves");
                            parsedResult += "Moves: " + moves + newLine;
                        }

                        parsedResult += newLine;
                    }
        ...
        private void AddAndToQuery(ref MySqlCommand query, int count)
        {
            if (count >= 1)
